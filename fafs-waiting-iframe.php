...
<script>
function loadDashboardForm(agent) {
    console.log("Loading dashboard form for agent " + agent);
    
    // 1. Set the AgentID in parent form (EXACTLY like call-start.php does)
    var aid = window.parent.document.getElementById('AgentID');
    if (aid) aid.value = agent;
    
    // 2. Update callData display (if exists)
    var el = window.parent.document.getElementById('callData');
    if (el) {
        el.style.display = "block";
        el.innerHTML = 'Agent:' + agent + ' CID: N/A ACD: N/A DID: N/A UID: MANUAL';
    }
    
    // 3. Show controls iframe (EXACT match to your working code)
    var acdsel = window.parent.document.getElementById('ifCall');
    if (acdsel) {
        acdsel.src = "fafs-controls.php";
        acdsel.style.display = "block";
    }
    
    // 4. Build the EXACT same URL format used in call-start.php
    var url = "https://dashboard.fafs.me/Admin/PBX/CallFormNew?AgentID=" + agent + 
              "&CID=N/A&DID=N/A&CallID=MANUAL&ANI=N/A&qnum=N/A&type=MANUAL" +
              "&serverip=" + encodeURIComponent("<?php echo MY_IP; ?>") + 
              "&holdsec=0&inout=MANUAL";
    
    console.log("Dashboard URL: " + url);
    
    // 5. Set cookie and redirect EXACTLY like your working version
    document.cookie = "myurl=" + encodeURIComponent(url);
